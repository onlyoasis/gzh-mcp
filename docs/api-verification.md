...
同一轮随后发现原草稿经 `draft/get` 回读的正文为 24061 个 Unicode 字符、28565
个 UTF-8 字节，本地同源转换 HTML 也有 23527 个字符；两者都超过旧的原始 HTML
`content < 20000` 校验。当前移除该错误字符数门槛，只保留正文非空、禁止 script
和拒绝非微信图片 URL，实际长度继续由微信平台判定，不裁剪正文。
